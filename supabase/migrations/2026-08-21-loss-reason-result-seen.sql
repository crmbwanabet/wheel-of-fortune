...
--                   pot_empty / queue_null distinguishable — before this all
--                   three were byte-identical losses.
...
BEGIN
  ...
  -- is one of the valid prize amounts. Guards against a malformed payload
  ...
  ELSIF p_payout_mode = 'queue' THEN
    ...
    ELSE
      ...
      ELSE
        ...
        -- element. 0 rows updated = pot empty / cap reached / no queue = loss,
        -- race-free.
        UPDATE wheel_daily_state
...
-- claim_spin is SECURITY DEFINER with p_skip_dedupe/p_force_prize — only the
-- server route (service_role) may execute it. Re-applied for safety.
