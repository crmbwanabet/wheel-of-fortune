...
// winner's slot out of WINNABLE_POSITIONS, so the ordinal line already
// carries it. In queue mode the win ordinal (winsToday of poolSize) is what
// decides the payout, so the spin sequence gets its own line underneath:
// the queue picks the winner, which makes the two numbers independent, and
// ops reconciles against both.
export function formatWinMessage({ customerId, prizeAmount, winsToday, budgetSpent, spinNumber, payoutMode = 'positions', poolSize = POOL_SIZE, game = 'wheel' }) {
  ...
  const queueMode = payoutMode === 'queue';
  const spinLine = queueMode
    ? `${icon} Win #${winsToday} of ${poolSize}`
    : `${icon} Spin: ${spinNumber}/${WINNABLE_POSITIONS}`;
  // The wheel-day's spin sequence (1..N, no gaps) that produced this win.
  // Deliberately NOT the game icon: on a box day a wheel icon here would
  // contradict the header that the whole GAME_LABELS table exists to get
  // right. The sequence is 1-based, so the > 0 test is what rejects a missing
  // number: Number(null) and Number('') are both 0, which is finite and would
  // otherwise print a spin that does not exist.
  const sequenceNumber = Number(spinNumber);
  const sequenceLine = queueMode && Number.isInteger(sequenceNumber) && sequenceNumber > 0
    ? [`🔢 Spin: ${sequenceNumber.toLocaleString('en-US')}`]
    : [];
  return [
    header,
    `👤 User ID: ${customerId}`,
