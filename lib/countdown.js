...
//
// The wheel-day resets at 09:00 CAT = 07:00 UTC. currentWheelDayStartUtc
// returns 04:00 UTC of the CURRENT wheel-day, so the next reset is always
// exactly one day after it. Reusing that helper keeps every wheel-day boundary
// in this codebase governed by one tested implementation rather than a second
// copy of the same date arithmetic.

import { currentWheelDayStartUtc } from './wheelTime.js';

const DAY_MS = 24 * 60 * 60 * 1000;

// Milliseconds until the next 07:00 UTC boundary. Always within (0, DAY_MS].
export function msUntilNextWheelReset(nowMs) {
  return currentWheelDayStartUtc(nowMs) + DAY_MS - nowMs;
}
