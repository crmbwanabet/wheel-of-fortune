...
import {
  TICKER_NAMES, TICKER_TOWNS, TICKER_PRIZES, JACKPOT_PRIZE,
  DELAY_MIN_MS, DELAY_SPREAD_MS, nextWinner, nextDelayMs,
} from './winnerTicker.js';

test('pools are large enough to feel varied', () => {
  assert.ok(TICKER_NAMES.length >= 30);
...
test('delay stays inside the jitter window', () => {
  for (let i = 0; i < 200; i++) {
    const d = nextDelayMs();
    assert.ok(d >= DELAY_MIN_MS && d < DELAY_MIN_MS + DELAY_SPREAD_MS, `delay ${d}`);
  }
  assert.equal(nextDelayMs(() => 0), DELAY_MIN_MS);
});
