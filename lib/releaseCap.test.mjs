...
// 07:00 UTC == 09:00 CAT == wheel hour 0.
...
test('wheel hour 0 starts at 07:00 UTC', () => {
  assert.equal(wheelHour(at('2026-08-20T07:00:00Z')), 0);
  assert.equal(wheelHour(at('2026-08-20T07:59:59Z')), 0);
  assert.equal(wheelHour(at('2026-08-20T08:00:00Z')), 1);
});

test('wheel hour rolls through UTC midnight to 23', () => {
  assert.equal(wheelHour(at('2026-08-20T23:30:00Z')), 16);
  assert.equal(wheelHour(at('2026-08-21T00:30:00Z')), 17);
  assert.equal(wheelHour(at('2026-08-21T06:59:59Z')), 23);
});

test('just before the reset is still hour 23 of the previous wheel-day', () => {
  assert.equal(wheelHour(at('2026-08-21T06:00:00Z')), 23);
});

test('cap at hour 0 is the first quota', () => {
  assert.equal(releaseCap(at('2026-08-20T07:30:00Z')), HOURLY_QUOTAS[0]);
});

test('cap is cumulative — unused early quota stays available', () => {
  assert.equal(
    releaseCap(at('2026-08-20T09:30:00Z')),
...
test('cap is monotonically non-decreasing across the day', () => {
  let prev = 0;
  for (let h = 0; h < 24; h++) {
    const cap = releaseCap(at('2026-08-20T07:00:00Z') + h * 3600_000);
...
test('final hour releases the entire pool', () => {
  assert.equal(releaseCap(at('2026-08-21T06:30:00Z')), POOL_SIZE);
});
