(function() {
  ...
  // --- Promo-site arrival popup ------------------------------------------
  // Visitors sent over by the free-spins promo sites carry ?wof=new|existing
  // on the landing URL (the affiliate 301 drops params, so the promo sites
  // link to the resolved signup/login URLs). Greet them once per session with
  // the same red Sunburst card they just saw win, telling them exactly what
  // to do next. Independent of the wheel widget on purpose: new players are
  // logged OUT, and the widget only builds after login.
  var ARRIVAL_KEY = 'bwanabet_wof_arrival_shown';
  var ARRIVAL_Z = 9996;               // below the wheel trigger (9998) and overlay (9999)
  var ARRIVAL_DELAY_MS = 800;

  function arrivalAudience() {
    var m = location.search.match(/[?&]wof=(new|existing)\b/);
    return m ? m[1] : null;
  }

  // Same-origin auth paths on a real BwanaBet host (so .co.zm visitors stay
  // on .co.zm); absolute bwanabet.com URLs anywhere else (the test harness).
  function arrivalBase() {
    return /(^|\.)bwanabet\.(com|co\.zm)$/.test(location.hostname) ? location.origin : 'https://bwanabet.com';
  }

  function initArrivalPopup() {
    try {
      var aud = arrivalAudience();
      if (!aud) return;
      ...
      css.textContent =
        '@keyframes bwWofBulbs{0%,100%{opacity:.95}50%{opacity:.4}}' +
        '@keyframes bwWofZoom{from{transform:scale(.7);opacity:0}to{transform:scale(1);opacity:1}}' +
        '@media (prefers-reduced-motion: reduce){#bwanabet-wof-card *{animation:none !important}}';
      document.head.appendChild(css);

      var scrim = document.createElement('div');
      scrim.id = 'bwanabet-wof-arrival';
      scrim.style.cssText = 'position:fixed;inset:0;z-index:' + ARRIVAL_Z + ';display:flex;' +
        'align-items:center;justify-content:center;background:rgba(4,2,12,.78);padding:16px;';

      var card = document.createElement('div');
      card.id = 'bwanabet-wof-card';
      card.style.cssText = 'position:relative;overflow:hidden;width:min(92vw,380px);text-align:center;' +
        'border-radius:20px;border:2px solid #e6b800;padding:26px 22px 22px;' +
        'background:radial-gradient(circle at 50% 34%, #c8102a 0%, #7a0212 58%, #3d0109 100%);' +
        'box-shadow:0 0 44px rgba(255,215,0,.25), 0 26px 70px rgba(0,0,0,.75);' +
        'font-family:"Roboto Condensed","Arial Narrow",Arial,sans-serif;color:#fff;' +
        'animation:bwWofZoom .45s cubic-bezier(.34,1.56,.64,1) both;';

      var bulbs = document.createElement('div');
      bulbs.style.cssText = 'position:absolute;inset:7px;border-radius:14px;pointer-events:none;' +
        'background:' +
        'repeating-linear-gradient(90deg,#ffd700 0 4px,transparent 4px 20px) top / 100% 4px no-repeat,' +
        'repeating-linear-gradient(90deg,#ffd700 0 4px,transparent 4px 20px) bottom / 100% 4px no-repeat,' +
        'repeating-linear-gradient(0deg,#ffd700 0 4px,transparent 4px 20px) left / 4px 100% no-repeat,' +
        'repeating-linear-gradient(0deg,#ffd700 0 4px,transparent 4px 20px) right / 4px 100% no-repeat;' +
        'filter:drop-shadow(0 0 4px rgba(255,215,0,.8));animation:bwWofBulbs 1.1s ease-in-out infinite;';

      var close = document.createElement('span');
      close.innerHTML = '&times;';
      close.setAttribute('role', 'button');
      close.setAttribute('aria-label', 'Close');
      close.style.cssText = 'position:absolute;top:8px;right:12px;font-size:22px;line-height:1;' +
        'color:rgba(255,255,255,.75);cursor:pointer;z-index:2;padding:4px;';

      var fifty = document.createElement('div');
      fifty.textContent = '50';
      fifty.style.cssText = 'position:relative;font-size:76px;font-weight:900;line-height:.85;margin-top:6px;' +
        'background:linear-gradient(180deg,#fff3b0 0%,#ffd700 45%,#e08b00 78%,#ffde6b 100%);' +
      ...
      msg.style.cssText = 'position:relative;font-size:16px;font-weight:700;color:rgba(255,255,255,.92);margin:12px 0 16px;';

      var cta = document.createElement('a');
      cta.textContent = btnText;
      cta.href = targetUrl;
      cta.style.cssText = 'position:relative;display:block;text-decoration:none;color:#1A1E2E;border-radius:11px;' +
        'padding:14px 18px;font-weight:900;text-transform:uppercase;letter-spacing:.05em;font-size:18px;' +
        'background:linear-gradient(180deg,#fff970 0%,#FEF200 40%,#e8b400 100%);' +
        'box-shadow:0 8px 28px rgba(254,242,0,.4), inset 0 1.5px 0 rgba(255,255,255,.7), inset 0 -3px 0 rgba(0,0,0,.18);';

      function dismiss() {
        try { sessionStorage.setItem(ARRIVAL_KEY, '1'); } catch (e) { /* ignore */ }
        if (scrim.parentNode) scrim.parentNode.removeChild(scrim);
      }
      close.addEventListener('click', dismiss);
      scrim.addEventListener('click', function (e) { if (e.target === scrim) dismiss(); });
      cta.addEventListener('click', function (e) {
        // Already on the signup/login page: closing is the action — navigating
        // would reload the form under them.
        dismiss();
        if (onTargetPage) { e.preventDefault(); }
      });

      card.appendChild(bulbs);
      card.appendChild(close);
      card.appendChild(fifty);
      ...
      function mount() {
        ...
        dbg('arrival popup shown for audience', aud);
      }
      setTimeout(function () {
        if (!document.body) {
    ...
    } catch (e) { dbg('arrival popup failed:', e && e.message); }
  }
  initArrivalPopup();
